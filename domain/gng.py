...
class GrowingNeuralGas:
    ...
    def __init__(self, input_data):
        ...
        self.data_width = input_data.shape[1]
    ...
    def fit_network(self, e_b, e_n, a_max, l, a, d, passes=1, plot_evolution=False):
        ...
        for p in range(passes):
            ...
            for observation in self.data:
                ...
                self.network._node[s_1]['error'] += spatial.distance.euclidean(observation, self.network._node[s_1]['vector'])**2
                ...
                update_w_s_1 = self.e_b * \
                    (np.subtract(observation,
                                 self.network._node[s_1]['vector']))
                self.network._node[s_1]['vector'] = np.add(
                    self.network._node[s_1]['vector'], update_w_s_1)

                for neighbor in self.network.neighbors(s_1):
                    update_w_s_n = self.e_n * \
                        (np.subtract(observation,
                                     self.network._node[neighbor]['vector']))
                    self.network._node[neighbor]['vector'] = np.add(
                        self.network._node[neighbor]['vector'], update_w_s_n)
                ...
                if steps % l == 0:
                    ...
                    error_max = 0
                    for u in self.network.nodes():
                        if self.network._node[u]['error'] > error_max:
                            error_max = self.network._node[u]['error']
                            q = u
                    ...
                    for u in self.network.neighbors(q):
                        if self.network._node[u]['error'] > largest_error:
                            largest_error = self.network._node[u]['error']
                            f = u
                    w_r = 0.5 * (np.add(self.network._node[q]['vector'], self.network._node[f]['vector']))
                    ...
                    self.network._node[q]['error'] *= a
                    self.network._node[f]['error'] *= a
                    self.network._node[r]['error'] = self.network._node[q]['error']
                # 9. decrease all error variables by multiplying them with a constant d
                error = 0
                for u in self.network.nodes():
                    error += self.network._node[u]['error']
    ...
    def get_nodes(self):
        vector = torch.empty(len(self.network.nodes), self.data_width)
        for u in self.network.nodes():
            vector[u] = self.network._node[u]['vector']
        return vector
    ...
    def plot_network(self, file_path):
        ...
        node_pos = {}
        for u in self.network.nodes():
            vector = self.network._node[u]['vector']
    ...
    def compute_global_error(self):
        ...
        for observation in self.data:
            nearest_units = self.find_nearest_units(observation)
            s_1 = nearest_units[0]
            global_error += spatial.distance.euclidean(observation, self.network._node[s_1]['vector'])**2
        return global_error
